...
module.exports = (customOptions) => {
  ...
  const { port, host, proxy, workspace } = mergedOptions
  ...
  assertFunc(options, 'init', function () {
    options.init(app, mergedOptions)
  })

  // parser
  app.use(getBodyParser(bodyParser, { timeout: 3000 }))
  app.use(getQueryParser())

  // before
  assertFunc(options, 'before', function () {
    options.before(app, mergedOptions)
  })
